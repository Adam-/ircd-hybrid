                     ...
                     int show_eon)
{
  ...
  {
    msg_init_numeric(&msg, &me, RPL_NAMREPLY, source_p);
    msg_push_str(&msg, "channel type", channel_pub_or_secret(chptr));
    msg_push(&msg, "channel", PARAM_TYPE_CHANNEL, chptr);
    msg_push_last(&msg);
    ...
    DLINK_FOREACH(ptr, chptr->members.head)
    {
      ...
      if (!uhnames)
        msg_push_fmt(&msg, "member", "%s%s", get_member_status(ms, multi_prefix), ms->client_p->name);
      else
        msg_push_fmt(&msg, "member", "%s%s!%s@%s", get_member_status(ms, multi_prefix), ms->client_p->name, ms->client_p->username, ms->client_p->host);
    }
  }

  msg_build(&msg);
  sendto_one_msg(source_p, &msg);
  msg_free(&msg);
