...
static int send_SSL_writev(SSL *ssl, const struct iovec *vec, int iovcnt)
{
  ...
  for (i = 0; i < iovcnt; ++i)
  {
    ...
    int ret = SSL_write(ssl, iov->iov_base, iov->iov_len);
...
send_queued_write(struct Client *to)
{
  ...
  {
    retlen = send_SSL_writev(to->localClient->fd.ssl, vec, i);
    if (retlen == -1)
      return;
  }
  else
