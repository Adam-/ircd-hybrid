...
m_invite(struct Client *source_p, int parc, char *parv[])
{
  ...
  if (parc < 2)
  {
    ...
    DLINK_FOREACH(node, source_p->connection->invited.head)
      sendto_one_numeric(source_p, RPL_INVITELIST,
                         ((struct Channel *)node->data)->name);

    sendto_one_numeric(source_p, RPL_ENDOFINVITELIST);
    return 0;
  }

  if (parc < 3 || EmptyString(parv[2]))
  {
    sendto_one_numeric(source_p, ERR_NEEDMOREPARAMS, "INVITE");
    return 0;
  }
  ...
  {
    sendto_one_numeric(source_p, ERR_NOSUCHNICK, parv[1]);
    return 0;
  }

  if ((chptr = hash_find_channel(parv[2])) == NULL)
  {
    sendto_one_numeric(source_p, ERR_NOSUCHCHANNEL, parv[2]);
    return 0;
  }
  ...
  {
    sendto_one_numeric(source_p, ERR_USERONCHANNEL, target_p->name, chptr->name);
    return 0;
  }
  ...
  {
    sendto_one_numeric(source_p, ERR_TOOMANYINVITE, chptr->name, "user");
    return 0;
  }
  ...
  sendto_one_numeric(source_p, RPL_INVITING, target_p->name, chptr->name);
