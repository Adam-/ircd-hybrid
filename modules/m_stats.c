...
report_confitem_types(struct Client *source_p, enum maskitem_type type)
{
  ...
  switch (type)
  {
    ...
    case CONF_ULINE:
      DLINK_FOREACH(node, uconf_items.head)
      {
        ...
        *p = '\0';
        ...
        sendto_one_numeric(source_p, RPL_STATSULINE, conf->name,
                           conf->user ? conf->user: "*",
      ...
      DLINK_FOREACH(node, cluster_items.head)
      {
        ...
        *p = '\0';
        ...
        sendto_one_numeric(source_p, RPL_STATSULINE, conf->name, "*", "*", buf);
      }
    ...
    case CONF_OPER:
      DLINK_FOREACH(node, oconf_items.head)
      {
        ...
        if (HasUMode(source_p, UMODE_OPER))
          sendto_one_numeric(source_p, RPL_STATSOLINE, 'O', conf->user, conf->host,
    ...
    case CONF_SERVICE:
      ...
      {
        conf = node->data;
        sendto_one_numeric(source_p, RPL_STATSSERVICE, 'S', "*", conf->name, 0, 0);
      }
      ...
      DLINK_FOREACH(node, server_items.head)
      {
        ...
        *p = '\0';
        ...
        if (!ConfigServerHide.hide_server_ips && HasUMode(source_p, UMODE_ADMIN))
          sendto_one_numeric(source_p, RPL_STATSCLINE, 'C', conf->host,
                             buf, conf->name, conf->port,
                             conf->class->name);
        else
          sendto_one_numeric(source_p, RPL_STATSCLINE, 'C',
...
stats_deny(struct Client *source_p, int parc, char *parv[])
{
  ...
  for (unsigned int i = 0; i < ATABLE_SIZE; ++i)
  {
    DLINK_FOREACH(node, atable[i].head)
    {
      ...
      sendto_one_numeric(source_p, RPL_STATSDLINE, 'D', conf->host, conf->reason);
    }
  }
}
...
stats_exempt(struct Client *source_p, int parc, char *parv[])
{
  ...
  for (unsigned int i = 0; i < ATABLE_SIZE; ++i)
  {
    DLINK_FOREACH(node, atable[i].head)
    {
      ...
      conf = arec->conf;
      sendto_one_numeric(source_p, RPL_STATSDLINE, 'e', conf->host, "");
    }
  }
}
  ...
  for (unsigned int i = 0; i < ATABLE_SIZE; ++i)
  {
    DLINK_FOREACH(node, atable[i].head)
    {
      ...
      if (arec->type == CONF_GLINE)
      {
        ...
        sendto_one_numeric(source_p, RPL_STATSKLINE, 'G',
                           conf->host ? conf->host : "*",
                           conf->user ? conf->user : "*",
                           conf->reason);
...
stats_hubleaf(struct Client *source_p, int parc, char *parv[])
{
  ...
  DLINK_FOREACH(node, server_items.head)
  {
    ...
    DLINK_FOREACH(dptr, conf->hub_list.head)
      sendto_one_numeric(source_p, RPL_STATSHLINE, 'H', dptr->data, conf->name, 0, "*");
  }
  ...
  DLINK_FOREACH(node, server_items.head)
  {
    ...
    DLINK_FOREACH(dptr, conf->leaf_list.head)
      sendto_one_numeric(source_p, RPL_STATSLLINE, 'L', dptr->data, conf->name, 0, "*");
  }
}
...
report_auth(struct Client *source_p, int parc, char *parv[])
{
  ...
  for (unsigned int i = 0; i < ATABLE_SIZE; ++i)
  {
    DLINK_FOREACH(node, atable[i].head)
    {
      ...
      sendto_one_numeric(source_p, RPL_STATSILINE, 'I',
...
report_Klines(struct Client *source_p, int tkline)
{
  ...
  for (unsigned int i = 0; i < ATABLE_SIZE; ++i)
  {
    DLINK_FOREACH(node, atable[i].head)
    {
      ...
      sendto_one_numeric(source_p, RPL_STATSKLINE, c, conf->host, conf->user,
                         conf->reason);
    }
  }
}
...
stats_tklines(struct Client *source_p, int parc, char *parv[])
{
  ...
  else if (ConfigGeneral.stats_k_oper_only == 1 && !HasUMode(source_p, UMODE_OPER))
  {
    ...
    sendto_one_numeric(source_p, RPL_STATSKLINE, 'k',
...
stats_uptime(struct Client *source_p, int parc, char *parv[])
{
  ...
  else
  {
    ...
    sendto_one_numeric(source_p, RPL_STATSUPTIME, now / 86400,
...
stats_class(struct Client *source_p, int parc, char *parv[])
{
  ...
  DLINK_FOREACH(node, class_get_list()->head)
  {
    ...
    sendto_one_numeric(source_p, RPL_STATSYLINE, 'Y',
...
stats_servlinks(struct Client *source_p, int parc, char *parv[])
{
  ...
  DLINK_FOREACH(node, local_server_list.head)
  {
    ...
    sendto_one_numeric(source_p, RPL_STATSLINKINFO,
  ...
  sendto_one_numeric(source_p, RPL_STATSDEBUG | SND_EXPLICIT, "? :%u total server(s)",
                     dlink_list_length(&local_server_list));
  sendto_one_numeric(source_p, RPL_STATSDEBUG | SND_EXPLICIT, "? :Sent total: %7.2f %s",
                     _GMKv(sendB), _GMKs(sendB));
  sendto_one_numeric(source_p, RPL_STATSDEBUG | SND_EXPLICIT, "? :Recv total: %7.2f %s",
...
stats_ltrace(struct Client *source_p, int parc, char *parv[])
{
  ...
  if ((name = parse_stats_args(source_p, parc, parv, &doall, &wilds)))
  {
    ...
    sendto_one_numeric(source_p, ERR_NEEDMOREPARAMS, "STATS");
}
...
do_stats(struct Client *source_p, int parc, char *parv[])
{
  ...
  sendto_one_numeric(source_p, RPL_ENDOFSTATS, statchar);
}
...
m_stats(struct Client *source_p, int parc, char *parv[])
{
  ...
  {
    sendto_one_numeric(source_p, RPL_LOAD2HI, "STATS");
    return 0;
  }
