...
m_knock(struct Client *source_p, int parc, char *parv[])
{
  ...
  {
    sendto_one_numeric(source_p, ERR_NEEDMOREPARAMS, "KNOCK");
    return 0;
  }
  ...
  {
    sendto_one_numeric(source_p, ERR_KNOCKONCHAN, chptr->name);
    return 0;
  }
  ...
  if (!((chptr->mode.mode & MODE_INVITEONLY) || chptr->mode.key[0] ||
        (chptr->mode.limit && dlink_list_length(&chptr->members) >=
         chptr->mode.limit)))
  {
    sendto_one_numeric(source_p, ERR_CHANOPEN, chptr->name);
    return 0;
  }
  ...
  if (MyClient(source_p))
  {
    ...
    {
      sendto_one_numeric(source_p, ERR_CANNOTSENDTOCHAN, chptr->name);
      return 0;
    }
    ...
    {
      sendto_one_numeric(source_p, ERR_TOOMANYKNOCK, chptr->name, "user");
      return 0;
    }

    if ((chptr->last_knock + ConfigChannel.knock_delay_channel) > CurrentTime)
    {
      sendto_one_numeric(source_p, ERR_TOOMANYKNOCK, chptr->name, "channel");
      return 0;
    }
    ...
    sendto_one_numeric(source_p, RPL_KNOCKDLVR, chptr->name);
  }
