...
remove_resv(struct Client *source_p, const char *name)
{
  ...
  if (IsChanPrefix(*name))
  {
    if ((conf = find_exact_name_conf(CONF_CRESV, NULL, name, NULL, NULL)) == NULL)
    {
      if (IsClient(source_p))
       sendto_one_notice(source_p, "A RESV does not exist for channel: %s", name);
    ...
    {
      if (IsClient(source_p))
        sendto_one_notice(source_p, "The RESV for channel: %s is in ircd.conf and must be removed by hand.",
                          name);
    ...
    if (IsClient(source_p))
      sendto_one_notice(source_p, "The RESV has been removed on channel: %s", name);
  ...
  else
  {
    ...
    {
      if (IsClient(source_p))
        sendto_one_notice(source_p, "A RESV does not exist for nick: %s", name);
    ...
    {
      if (IsClient(source_p))
        sendto_one_notice(source_p, "The RESV for nick: %s is in ircd.conf and must be removed by hand.",
                          name);
    ...
    if (IsClient(source_p))
      sendto_one_notice(source_p, "The RESV has been removed on nick: %s", name);
