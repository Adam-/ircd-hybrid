       ...
       const char *name, const char *op_flags)
{
  ...
  if (ConfigServerHide.hide_servers || IsHidden(target_p->servptr))
    sendto_one_numeric(source_p, RPL_WHOREPLY,
               (name) ? (name) : "*",
  ...
  DLINK_FOREACH(node, chptr->members.head)
  {
      ...
      !match(mask, target_p->info))
    {
      ...
      if (*maxmatches > 0)
      {
        if (--(*maxmatches) == 0)
        {
          sendto_one_numeric(source_p, ERR_WHOLIMEXCEED, WHO_MAX_REPLIES, "WHO");
          return;
        }
      }
    }
...
who_global(struct Client *source_p, char *mask, int server_oper)
{
  ...
  if (!HasUMode(source_p, UMODE_OPER))
  {
    ...
    {
      sendto_one_numeric(source_p, RPL_LOAD2HI, "WHO");
      return;
    }
...
m_who(struct Client *source_p, int parc, char *parv[])
{
  ...
  if (EmptyString(mask))
  {
    ...
    sendto_one_numeric(source_p, RPL_ENDOFWHO, "*");
    return 0;
  }
  ...
  if (IsChanPrefix(*mask))
  {
    ...
    sendto_one_numeric(source_p, RPL_ENDOFWHO, mask);
    return 0;
  }
