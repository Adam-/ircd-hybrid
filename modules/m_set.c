...
quote_autoconn(struct Client *source_p, const char *arg, int newval)
{
  if (!EmptyString(arg))
  {
    ...
    if (conf)
    {
      ...
      sendto_one_notice(source_p, "AUTOCONN for %s is now set to %i",
                        arg, newval);
    }
    else
      sendto_one_notice(source_p, "Cannot find %s", arg);
  }
  else
    sendto_one_notice(source_p, "Please specify a server name!");
}
...
quote_max(struct Client *source_p, const char *arg, int newval)
{
  if (newval > 0)
  {
    ...
    {
      sendto_one_notice(source_p, "You cannot set MAXCLIENTS to > %d, restoring to %d",
    ...
    sendto_one_notice(source_p, "Current MAXCLIENTS = %d (%d)",
...
quote_splitnum(struct Client *source_p, const char *arg, int newval)
{
  if (newval >= 0)
  {
    ...
    sendto_one_notice(source_p, "SPLITNUM is currently %i",
                      split_servers);
}
...
quote_splitusers(struct Client *source_p, const char *arg, int newval)
{
  if (newval >= 0)
  {
    ...
    sendto_one_notice(source_p, "SPLITUSERS is currently %i",
                      split_users);
}
...
quote_jfloodtime(struct Client *source_p, const char *arg, int newval)
{
  if (newval >= 0)
  {
    ...
    sendto_one_notice(source_p, "JFLOODTIME is currently %i",
...
quote_jfloodcount(struct Client *source_p, const char *arg, int newval)
{
  if (newval >= 0)
  {
    ...
    sendto_one_notice(source_p, "JFLOODCOUNT is currently %i",
...
mo_set(struct Client *source_p, int parc, char *parv[])
{
  ...
  if (parc > 1)
  {
    ...
    for (const struct SetStruct *tab = set_cmd_table; tab->handler; ++tab)
    {
      ...
      if ((n - 1) > parc)
        sendto_one_notice(source_p, "SET %s expects (\"%s%s\") args", tab->name,
      ...
      if (tab->wants_int && parc > 2)
      {
        ...
        if (newval < 0)
        {
          sendto_one_notice(source_p, "Value less than 0 illegal for %s",
                            tab->name);
          return 0;
        }
      }
