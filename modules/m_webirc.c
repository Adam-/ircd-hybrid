...
mr_webirc(struct Client *source_p, int parc, char *parv[])
{
  ...
  {
    sendto_one_notice(source_p, &me.client, ":CGI:IRC: Invalid hostname");
    return 0;
  }
  ...
  {
    sendto_one_notice(source_p, &me.client, ":Not a CGI:IRC auth block");
    return 0;
  }

  if (EmptyString(conf->passwd))
  {
    sendto_one_notice(source_p, &me.client, ":CGI:IRC auth blocks must have a password");
    return 0;
  }

  if (!match_conf_password(parv[1], conf))
  {
    sendto_one_notice(source_p, &me.client, ":CGI:IRC password incorrect");
    return 0;
  }
  ...
  {
    sendto_one_notice(source_p, &me.client, ":Invalid CGI:IRC IP %s", parv[4]);
    return 0;
  }
  ...
  sendto_one_notice(source_p, &me.client, ":CGI:IRC host/IP set to %s %s",
                    parv[3], parv[4]);
  return 0;
}
