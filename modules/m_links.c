...
do_links(struct Client *source_p, int parc, char *parv[])
{
  ...
  if (HasUMode(source_p, UMODE_OPER) || !ConfigServerHide.flatten_links)
  {
    ...
    DLINK_FOREACH(node, global_server_list.head)
    {
       ...
       * or they're an oper..
       */
      sendto_one_numeric(source_p, RPL_LINKS,
                         target_p->name, target_p->servptr->name,
                         target_p->hopcount, target_p->info);
    }

    sendto_one_numeric(source_p, RPL_ENDOFLINKS,
  ...
  else
  {
     ...
     */
    sendto_one_numeric(source_p, RPL_LINKS, me.name, me.name, 0, me.info);

    DLINK_FOREACH(node, flatten_links.head)
      sendto_one_numeric(source_p, RPL_LINKS | SND_EXPLICIT, "%s", node->data);
    sendto_one_numeric(source_p, RPL_ENDOFLINKS, "*");
  }
}
...
m_links(struct Client *source_p, int parc, char *parv[])
{
  ...
  {
    sendto_one_numeric(source_p, RPL_LOAD2HI, "LINKS");
    return 0;
  }
